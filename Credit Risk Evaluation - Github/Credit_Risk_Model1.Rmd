...
png("error1_importance_plot.png", width = 400, height = 350)  # Adjust width and height in pixels

#Plot the errors
plot(model, main = "Overall Error - Model 1")
...
imp_plot <- importance(model)

png("variable_importance_plot.png", width = 400, height = 350)  # Adjust width and height in pixels

# Create the plot using varImpPlot()
varImpPlot(model, main = "Attribute Importances")
...
Avaliar o MCC para dados desbalanceado [Ler sobre]

# Prediction

```{r prediction, echo=FALSE, error=TRUE}

df.pred <- import_data("./RAW Data/german_pred.csv", ";")

new_pred <- predict(model, newdata = df.pred)

#print(paste("Credit Risk:", new_prediction))

df.pred <- data.frame(prediction = new_pred)
df.pred$New_Person_ID <- seq(from = 1, to = length(df.pred$pred))
